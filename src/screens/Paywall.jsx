          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-6">
            ...
            {plans !== null && plans.length > 0 && (
              <div className="grid gap-3" style={{ gridTemplateColumns: "repeat(auto-fill,minmax(180px,1fr))" }}>
                {plans.map(p => (
                  <div key={p.priceId} className="rounded-xl border border-slate-200 p-4 flex flex-col">
                    <div className="font-semibold text-slate-800">{p.name}</div>
                    <div className="text-2xl font-bold text-slate-800">{fmtPrice(p)}<span className="text-xs font-normal text-slate-400">{p.amount ? perInterval(p) : ""}</span></div>
                    {p.description && <div className="text-[11px] text-slate-500 my-1 leading-snug">{p.description}</div>}
                    {p.seats != null && <div className="text-[10px] text-slate-400 mb-1.5">Up to {p.seats} seats</div>}
                    <div className="mt-auto pt-3"><Btn variant="dark" className="w-full justify-center" onClick={() => subscribe(p.priceId)} disabled={busy}>Subscribe</Btn></div>
                  </div>
                ))}
              </div>
            )}
